...
public class SqlToOperationConverter {
    ...
    public static Optional<Operation> convert(
        ...
        } else if (validated instanceof SqlCreateDebug){
            return Optional.of(converter.convertCreateDebug((SqlCreateDebug) validated));
    ...
    /** Convert CREATE DEBUG statement. */
    private Operation convertCreateDebug(SqlCreateDebug sqlCreateDebug) {
        final SqlNode query = sqlCreateDebug.getQuery();
        final SqlNodeList fieldList = sqlCreateDebug.getFieldList();

        UnresolvedIdentifier unresolvedIdentifier = UnresolvedIdentifier.of(sqlCreateDebug.fullDebugName());
        ObjectIdentifier identifier = catalogManager.qualifyIdentifier(unresolvedIdentifier);

        String comment = sqlCreateDebug.getComment()
                .map(c -> c.getNlsString().getValue()).orElse(null);
        CatalogDebug catalogdebug = convertDebugQuery(
                query,
                fieldList.getList(),
                OperationConverterUtils.extractProperties(sqlCreateDebug.getProperties()
                        .orElse(null)),
                comment);
        return new CreateDebugOperation(
                identifier,
                catalogdebug,
                sqlCreateDebug.isIfNotExists(),
                sqlCreateDebug.isTemporary());
    }

    /** Convert the query part of a DEBUG statement. */
    private CatalogDebug convertDebugQuery(SqlNode query, List<SqlNode> fieldNames,
                                           Map<String, String> props, String comment) {
        // Put the sql string unparse (getQuotedSqlString()) in front of
        // the node conversion (toQueryOperation()),
        // because before Calcite 1.22.0, during sql-to-rel conversion, the SqlWindow
        ...
        // the debug column list in CREATE DEBUG is optional, if it's not empty, we should update
        // the column name with the names in debug column list.
        if (!fieldNames.isEmpty()) {
            // alias column names:
            String[] inputFieldNames = schema.getFieldNames();
            String[] aliasFieldNames = fieldNames.stream()
            ...
            if (inputFieldNames.length != aliasFieldNames.length) {
                throw new ValidationException(String.format(
                        "DEBUG definition and input fields not match:\n\tDef fields: %s.\n\tInput fields: %s.",
                        Arrays.toString(aliasFieldNames), Arrays.toString(inputFieldNames)));
            }
        ...
        return new CatalogDebugImpl(originalQuery,
                expandedQuery,
                schema,
                props,
                comment);
