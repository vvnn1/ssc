...
public final class FileUtils {
    ...
    public static boolean exists(Path path) throws IOException {
        FileSystem pFs = FileSystem.getUnguardedFileSystem(path.toUri());
    ...
    public static boolean deleteFile(Path path) throws IOException{
        ...
        try {
            fileStatus = pFS.getFileStatus(path);
        }
        catch (FileNotFoundException e) {
            return true;
        }
        ...
        if (fileStatus.isDir()) {
            throw new UnsupportedFileTypeException("unsupported delete direct:" + path.toString());
        }

        return pFS.delete(path, false);
    }
    ...
    public static void copyFile(InputStream is, Path targetPath) throws IOException{
        FileSystem tFs = FileSystem.getUnguardedFileSystem(targetPath.toUri());
        if (tFs.exists(targetPath)){
            throw new FileAlreadyExistsException("target file " + targetPath.toString() + " already exists");
        }
        internalCopyFile(targetPath, tFs, is);
    }

    public static void copyFile(Path sourcePath, OutputStream os) throws IOException{
        FileSystem sFS = FileSystem.getUnguardedFileSystem(sourcePath.toUri());
        if (!sFS.exists(sourcePath)) {
            throw new FileNotFoundException("source file " + sourcePath.toString() + " not exists");
        }

        if (sFS.getFileStatus(sourcePath).isDir()) {
            throw new UnsupportedFileTypeException("unsupported direct file:" + sourcePath.toString());
        }
        internalCopyFile(sourcePath, sFS, os);
    }
    ...
    private static void internalCopyFile(Path targetPath, FileSystem tFS, InputStream inputStream) throws IOException{
        try (FSDataOutputStream lfsOutput = tFS.create(targetPath, FileSystem.WriteMode.NO_OVERWRITE); InputStream fsInput = inputStream){
            IOUtils.copyBytes(fsInput, lfsOutput);
        }
    }

    private static void internalCopyFile(Path sourcePath, FileSystem sFs, OutputStream outputStream) throws IOException{
        try (OutputStream lfsOutput = outputStream; FSDataInputStream fsInput = sFs.open(sourcePath)){
            IOUtils.copyBytes(fsInput, lfsOutput);
        }
    }
